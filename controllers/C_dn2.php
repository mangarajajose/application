...
class C_dn2 extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_dn2'); // Load model
	    ...
	    $Hak_akses = $this->UserModel->get_controller_access($this->session->userdata('role_id'),'C_dn2'); 
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_dn2->get_tb_dn2();  // Menarik data dan menampung nya menjadi groups
        // var_dump($data);
        // die;
		...
		$data['company_to'] = $this->M_dn2->Tampil_company_to();// Menarik dan menampung ke data company to
		$data['currency'] = $this->M_dn2->Tampil_Currency();// Menarik dan menampung ke data currency
		$data['vat'] = $this->M_dn2->Tampil_vat();// Menarik dan menampung ke data vat
        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar',$data_akses); //Tampil Sidebar
		// // $this->load->view('dn2/V_dn2',$data); // Tampil data
        $this->load->view('dn2/V_dn2',$data); // Tampil data
     ...
     function view_data()
     {
		...
		$search = array('hdrid','date_of_issue','name','nik','section','company_to','company_address','company_address1','company_address2','attention','attention_departement','email_and_telp','detail_request','reference','budget','account','payment_terms','transfer_to','transfer_address','currency','idr','usd','eur','jpy','swift_code','account_name','dpp','vat_persen','vat_nominal','amount_total','amount_in_word','attachment_1','attachment_2','attachment_3','attachment_4','director','title');
         ...
         echo $this->M_dn2->get_tables($tables,$search,$isWhere);  // Mengambil data dari database
    ...
    {
		$tables = "tb_dn2";
		$search = array('hdrid','date_of_issue','name','nik','section','company_to','company_address','company_address1','company_address2','attention','attention_departement','email_and_telp','detail_request','reference','budget','account','payment_terms','transfer_to','transfer_address','currency','idr','usd','eur','jpy','swift_code','account_name','dpp','vat_persen','vat_nominal','amount_total','amount_in_word','attachment_1','attachment_2','attachment_3','attachment_4','director','title','status');
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');  // Memberi tahu browser bahwa data dalam bentuk format json
        echo $this->M_dn2->get_tables_where($tables,$search,$where,$isWhere);  // Mengambil data dari database
    ...
    public function ajax_add()
	{
        ...
        $mdate="DN".mdate('%Y%m',time());   // Buat HDRID otomatis dengan format TR Tahun Bulan Jam (TR202210)         
        $hdrid2=$this->M_dn2->Max_data($mdate,'tb_dn2')->row();  // Mengambil row dari database        
        if ($hdrid2->hdrid==NULL){ // Jika HDRID belum ada
           $hdrid3=$mdate."001"; // Maka mulai dari 001
        }else{
           $hdrid3=$hdrid2->hdrid; // Jika sudah ada 
           $hdrid3="DN".(intval(substr($hdrid3,2,10))+1); // Jika sudah ada maka naik 1 level
        }
        ...
        $this->M_dn2->Input_Data($post_datamerge,'tb_dn2');  // Mengirim parameter ke model untuk query 

        // ********************* 6. Upload file jika ada  *********************   
		if(!empty($_FILES['attachment_1']['name']))
		{
		  $this->upload_file_attach('attachment_1',$hdrid,'tb_dn2');
		}
			if(!empty($_FILES['attachment_2']['name']))
					{
					$this->upload_file_attach('attachment_2',$hdrid,'tb_dn2');
					}
			if(!empty($_FILES['attachment_3']['name']))
					{
					$this->upload_file_attach('attachment_3',$hdrid,'tb_dn2');
					}
			if(!empty($_FILES['attachment_4']['name']))
					{
					$this->upload_file_attach('attachment_4',$hdrid,'tb_dn2');
					}
...
// 		}
        ...
        $this->M_dn2->Update_Data($where,$post_datamerge,'tb_dn2');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_dn2->Delete_Data($where,'tb_dn2'); // Mengirim parameter ke model 
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_dn2->ajax_getbyhdrid($hdrid,'tb_dn2')->row();  // Mengirim parameter ke model dan menampung nya menjadi variable
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_dn2->get_approver($id_user_reg); // Menarik data hasil query dan menampung nya sebagai data get_approver 
		$data['get_requester']=$this->M_dn2->get_requester($id_user_reg); // Menarik data hasil query dan menampung nya sebagai data get_requester
		$data['data']=$this->M_dn2->get_data($id_user_reg);  // Menarik data hasil query dan menampung nya sebagai data 
        
        $this->load->view('email/V_dn2',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            $config['upload_path'] = './assets/upload/dn2/'; // Configurasi folder yang ingin dijadikan penyimpanan untuk upload file  
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $this->M_dn2->Update_Data($where,$data_update,$table); // Kirim semua parameter ke model
                     ...
                     if ($index > 1 && ucwords($value['D'])!='' ) {
                        ...
                        // ********************* 0. Generate nomor transaksi  *********************          
                            ...
                            if($index==2){
                                ...
                                $hdrid2=$this->M_dn2->Max_data($mdate,'tb_dn2')->row();  
							...
							$resultData[$index]['jpy'] = ucwords($value['A']);	
