...
class C_akun_dn extends CI_Controller {
    ...
    public function __construct(){
        ...
        $this->load->model('M_akun_dn'); // Load model
        ...
        $Hak_akses = $this->UserModel->get_controller_access($this->session->userdata('role_id'),'C_akun_dn'); 
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_akun_dn->get_tb_akun_dn(); // Menarik dan menampung ke data groups
        // var_dump($data);
        // die;
		...
		// // $this->load->view('akun_dn/V_akun_dn',$data); // Tampil data
        $this->load->view('akun_dn/V_akun_dn',$data); // Tampil data
    ...
    function view_data()
    {
        ...
        echo $this->M_akun_dn->get_tables($tables,$search,$isWhere); // Mengambil data dari database
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');  // Memberi tahu browser bahwa data dalam bentuk format json 
        echo $this->M_akun_dn->get_tables_where($tables,$search,$where,$isWhere); // Mengambil data dari database     
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_akun_dn->Max_data($mdate,'tb_akun_dn')->row(); // Mengambil row dari database           
        ...
        $this->M_akun_dn->Input_Data($post_datamerge,'tb_akun_dn'); // Kirim hasil gabungan data ke model untuk insert tb_akun_dn

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))    
        {
            $this->upload_file_attach('file',$hdrid,'tb_akun_dn');  // Upload file attach ke table tb_akun_dn     
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_akun_dn->Update_Data($where,$post_datamerge,'tb_akun_dn'); // Kirim hasil gabungan,kondisi sesuai hdrid data ke model untuk update tb_akun_dn
    ...
    public function ajax_delete()
	{
        ...
        $this->M_akun_dn->Delete_Data($where,'tb_akun_dn'); // Kirim kondisi where,di table tb_akun_dn untuk query di model
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_akun_dn->ajax_getbyhdrid($hdrid,'tb_akun_dn')->row(); // Kirim $hdrid,di table tb_akun_dn untuk query di model
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $this->M_akun_dn->Update_Data($where,$data_update,$table); // Kirim semua parameter ke model
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_akun_dn'); // Kembali ke halaman C_akun_dn
			...
			if ( ! $this->upload->do_upload('excel')){
                    ...
                    if ($index > 1 && ucwords($value['A'])!='' ) {
                        ...
                        // ********************* 0. Generate nomor transaksi  *********************          
                            ...
                            if($index==2){
                                ...
                                $hdrid2=$this->M_akun_dn->Max_data($mdate,'tb_akun_dn')->row();  
				...
				if (count($resultData) != 0) {
					$result = $this->M_akun_dn->insert_batch('tb_akun_dn',$resultData);
					if ($result > 0) {
