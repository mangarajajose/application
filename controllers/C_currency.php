...
class C_currency extends CI_Controller {
    ...
    public function __construct(){
        ...
        $this->load->model('M_currency'); // Load model
        ...
        $Hak_akses = $this->UserModel->get_controller_access($this->session->userdata('role_id'),'C_currency'); 
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_currency->get_tb_currency(); // Menarik dan menampung ke data groups
        // var_dump($data);
        // die;
		...
		// // $this->load->view('currency/V_currency',$data); // Tampil data
        $this->load->view('currency/V_currency',$data); // Tampil data
    ...
    function view_data()
    {
        ...
        $tables = "tb_currency"; // Datatables yang akan ditampilkan        
        $search = array('hdrid','currency'); // Column-column pencarian di feature search datatables
        ...
        echo $this->M_currency->get_tables($tables,$search,$isWhere); // Mengambil data dari database
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');  // Memberi tahu browser bahwa data dalam bentuk format json 
        echo $this->M_currency->get_tables_where($tables,$search,$where,$isWhere); // Mengambil data dari database     
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_currency->Max_data($mdate,'tb_currency')->row(); // Mengambil row dari database           
        ...
        $this->M_currency->Input_Data($post_datamerge,'tb_currency'); // Kirim hasil gabungan data ke model untuk insert tb_currency

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))    
        {
            $this->upload_file_attach('file',$hdrid,'tb_currency');  // Upload file attach ke table tb_currency     
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_currency->Update_Data($where,$post_datamerge,'tb_currency'); // Kirim hasil gabungan,kondisi sesuai hdrid data ke model untuk update tb_currency
    ...
    public function ajax_delete()
	{
        ...
        $this->M_currency->Delete_Data($where,'tb_currency'); // Kirim kondisi where,di table tb_currency untuk query di model
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_currency->ajax_getbyhdrid($hdrid,'tb_currency')->row(); // Kirim $hdrid,di table tb_currency untuk query di model
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $this->M_currency->Update_Data($where,$data_update,$table); // Kirim semua parameter ke model
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_currency'); // Kembali ke halaman C_currency
			...
			if ( ! $this->upload->do_upload('excel')){
                    ...
                    if ($index > 1 && ucwords($value['A'])!='' ) {
                        ...
                        // ********************* 0. Generate nomor transaksi  *********************          
                            ...
                            if($index==2){
                                ...
                                $hdrid2=$this->M_currency->Max_data($mdate,'tb_currency')->row();  
				...
				if (count($resultData) != 0) {
					$result = $this->M_currency->insert_batch('tb_currency',$resultData);
					if ($result > 0) {
