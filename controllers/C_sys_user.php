...
class C_sys_user extends CI_Controller {
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_sys_user->get_tb_sys_user();
        // var_dump($data);
        // die;

        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('sys_user/V_sys_user',$data); // Tampil data
        $this->load->view('sys_user/V_sys_user',$data); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_sys_user";        
        $search = array('hdrid','nik','user_name','section_name','section_code','email','role_id','is_active','role_name');
         ...
         echo $this->M_sys_user->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_sys_user->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_sys_user->Max_data($mdate,'tb_sys_user')->row();        
        ...
        $this->M_sys_user->Input_Data($post_datamerge,'tb_sys_user');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_sys_user');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_sys_user->Update_Data($where,$post_datamerge,'tb_sys_user');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_sys_user->Delete_Data($where,'tb_sys_user');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_sys_user->ajax_getbyhdrid($hdrid,'tb_sys_user')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_sys_user->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_sys_user->get_requester($id_user_reg); 
		$data['data']=$this->M_sys_user->get_data($id_user_reg); 
        
        $this->load->view('email/V_sys_user',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_sys_user->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				foreach ($sheetData as $key => $value) {
                     ...
                     if ($index > 1 && ucwords($value['D'])!='' ) {
                        ...
                        // ********************* 0. Generate nomor transaksi  *********************          
                            ...
                            $resultData[$index]['user_name'] = ucwords($value['A']);
                            ...
                            $resultData[$index]['role_id'] = ucwords($value['A']);
                            $resultData[$index]['is_active'] = ucwords($value['A']);
                            $resultData[$index]['role_name'] = ucwords($value['A']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_sys_user->insert_batch('tb_sys_user',$resultData);
					if ($result > 0) {
