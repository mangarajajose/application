...
class C_sys_role extends CI_Controller {
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_sys_role->get_tb_sys_role();
        // var_dump($data);
        // die;

        $this->load->view('templates/header'); //Tampil header
		$this->load->view('templates/sidebar'); //Tampil Sidebar
		// // $this->load->view('sys_role/V_sys_role',$data); // Tampil data
        $this->load->view('sys_role/V_sys_role',$data); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_sys_role";        
        $search = array('hdrid','role_id','role_name');
         ...
         echo $this->M_sys_role->get_tables($tables,$search,$isWhere);
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');
        echo $this->M_sys_role->get_tables_where($tables,$search,$where,$isWhere);
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_sys_role->Max_data($mdate,'tb_sys_role')->row();        
        ...
        $this->M_sys_role->Input_Data($post_datamerge,'tb_sys_role');

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_sys_role');           
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_sys_role->Update_Data($where,$post_datamerge,'tb_sys_role');
    ...
    public function ajax_delete()
	{
        ...
        $this->M_sys_role->Delete_Data($where,'tb_sys_role');
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_sys_role->ajax_getbyhdrid($hdrid,'tb_sys_role')->row();
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_sys_role->get_approver($id_user_reg); 
		$data['get_requester']=$this->M_sys_role->get_requester($id_user_reg); 
		$data['data']=$this->M_sys_role->get_data($id_user_reg); 
        
        $this->load->view('email/V_sys_role',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $where = array('hdrid' => $hdrid);        
                $this->M_sys_role->Update_Data($where,$data_update,$table);
		...
		if ($_FILES['excel']['name'] == '') {
			...
			if ( ! $this->upload->do_upload('excel')){
				...
				if (count($resultData) != 0) {
					$result = $this->M_sys_role->insert_batch('tb_sys_role',$resultData);
					if ($result > 0) {
