...
class C_cicilan_dn extends CI_Controller {
    ...
    public function __construct(){
        ...
        $this->load->model('M_cicilan_dn'); // Load model
        ...
        $Hak_akses = $this->UserModel->get_controller_access($this->session->userdata('role_id'),'C_cicilan_dn'); 
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_cicilan_dn->get_tb_cicilan_dn(); // Menarik dan menampung ke data groups
        // var_dump($data);
        // die;
		...
		// // $this->load->view('cicilan_dn/V_cicilan_dn',$data); // Tampil data
        $this->load->view('cicilan_dn/V_cicilan_dn',$data); // Tampil data
    ...
    function view_data()
    {
        ...
        $tables = "tb_cicilan_dn";
        $search = array('hdrid','payment_terms','month','amount');
        ...
        echo $this->M_cicilan_dn->get_tables($tables,$search,$isWhere); // Mengambil data dari database
   ...
   /// @see view_data_where()
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');  // Memberi tahu browser bahwa data dalam bentuk format json 
        echo $this->M_cicilan_dn->get_tables_where($tables,$search,$where,$isWhere); // Mengambil data dari database     
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_cicilan_dn->Max_data($mdate,'tb_cicilan_dn')->row(); // Mengambil row dari database           
        ...
        $this->M_cicilan_dn->Input_Data($post_datamerge,'tb_cicilan_dn'); // Kirim hasil gabungan data ke model untuk insert tb_cicilan_dn

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))    
        {
            $this->upload_file_attach('file',$hdrid,'tb_cicilan_dn');  // Upload file attach ke table tb_cicilan_dn     
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_cicilan_dn->Update_Data($where,$post_datamerge,'tb_cicilan_dn'); // Kirim hasil gabungan,kondisi sesuai hdrid data ke model untuk update tb_cicilan_dn
    ...
    public function ajax_delete()
	{
        ...
        $this->M_cicilan_dn->Delete_Data($where,'tb_cicilan_dn'); // Kirim kondisi where,di table tb_cicilan_dn untuk query di model
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_cicilan_dn->ajax_getbyhdrid($hdrid,'tb_cicilan_dn')->row(); // Kirim $hdrid,di table tb_cicilan_dn untuk query di model
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_cicilan_dn->get_approver($id_user_reg); // Menarik data hasil query dan menampung nya sebagai data get_approver
		$data['get_requester']=$this->M_cicilan_dn->get_requester($id_user_reg); // Menarik data hasil query dan menampung nya sebagai data get_requester
		$data['data']=$this->M_cicilan_dn->get_data($id_user_reg); // Menarik data hasil query dan menampung nya sebagai data 
        
        $this->load->view('email/V_cicilan_dn',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $this->M_cicilan_dn->Update_Data($where,$data_update,$table); // Kirim semua parameter ke model
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_cicilan_dn'); // Kembali ke halaman C_cicilan_dn
			...
			if ( ! $this->upload->do_upload('excel')){
                    ...
                    if ($index > 1 && ucwords($value['A'])!='' ) {
                        ...
                        // ********************* 0. Generate nomor transaksi  *********************          
                            ...
                            if($index==2){
                                ...
                                $hdrid2=$this->M_cicilan_dn->Max_data($mdate,'tb_cicilan_dn')->row();  
				...
				if (count($resultData) != 0) {
					$result = $this->M_cicilan_dn->insert_batch('tb_cicilan_dn',$resultData);
					if ($result > 0) {
