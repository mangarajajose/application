...
class C_role_form extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_role_form'); // Load model
	    ...
	    $Hak_akses = $this->UserModel->get_controller_access($this->session->userdata('role_id'),'C_role_form'); 
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_role_form->get_tb_role_form();
        // var_dump($data);
        // die;
		...
		// // $this->load->view('role_form/V_role_form',$data); // Tampil data
        $this->load->view('role_form/V_role_form',$data); // Tampil data
     ...
     function view_data()
     {
        ...
        $tables = "tb_role_form"; // Table yang digunakan       
        $search = array('hdrid','nik','nama','role','email'); // Column column yang akan muncul di feature search datatable
         ...
         echo $this->M_role_form->get_tables($tables,$search,$isWhere); // Mengambil data dari database
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json'); // Memberi tahu browser bahwa data dalam bentuk format json
        echo $this->M_role_form->get_tables_where($tables,$search,$where,$isWhere); // Mengambil data dari database
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_role_form->Max_data($mdate,'tb_role_form')->row(); // Mengambil row dari database  
        ...
        $this->M_role_form->Input_Data($post_datamerge,'tb_role_form'); // Mengirim parameter ke model untuk query 

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_role_form'); // Upload attach file sesuai table             
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_role_form->Update_Data($where,$post_datamerge,'tb_role_form'); // Mengirim parameter ke model
    ...
    public function ajax_delete()
	{
        ...
        $this->M_role_form->Delete_Data($where,'tb_role_form'); // Mengirim parameter ke model 
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_role_form->ajax_getbyhdrid($hdrid,'tb_role_form')->row(); // Mengirim parameter ke model dan menampung nya menjadi variable
    ...
    function form_approver_link_mail(){
        ...
        $data['get_approver']=$this->M_role_form->get_approver($id_user_reg); // Menarik data hasil query dan menampung nya sebagai data get_approver 
		$data['get_requester']=$this->M_role_form->get_requester($id_user_reg); // Menarik data hasil query dan menampung nya sebagai data get_requester
		$data['data']=$this->M_role_form->get_data($id_user_reg);  // Menarik data hasil query dan menampung nya sebagai data 
        
        $this->load->view('email/V_role_form',$data); // Tampil data
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $this->M_role_form->Update_Data($where,$data_update,$table); // Kirim semua parameter ke model
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_role_form'); // Kembali ke halaman C_role_form
			...
			if ( ! $this->upload->do_upload('excel')){
                     ...
                     if ($index > 1 && ucwords($value['D'])!='' ) {
                        ...
                        // ********************* 0. Generate nomor transaksi  *********************          
                            ...
                            if($index==2){
                                ...
                                $hdrid2=$this->M_role_form->Max_data($mdate,'tb_role_form')->row();  
                            ...
                            $resultData[$index]['nama_role_form'] = ucwords($value['A']);// Isi nama_role_form dari column A excel 
				...
				if (count($resultData) != 0) {
					$result = $this->M_role_form->insert_batch('tb_role_form',$resultData);
					if ($result > 0) {
