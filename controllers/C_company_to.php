...
class C_company_to extends CI_Controller {
    ...
    public function __construct(){
        ...
        $this->load->model('M_company_to'); // Load model
        ...
        $Hak_akses = $this->UserModel->get_controller_access($this->session->userdata('role_id'),'C_company_to'); 
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_company_to->get_tb_company_to(); // Menarik dan menampung ke data groups
        // var_dump($data);
        // die;
		...
		// // $this->load->view('company_to/V_company_to',$data); // Tampil data
        $this->load->view('company_to/V_company_to',$data); // Tampil data
    ...
    function view_data()
    {
        ...
        $tables = "tb_company_to";
        $search = array('hdrid','company_to','company_address','company_address1','company_address2','attention','attention_departement','email_and_telp');
        ...
        echo $this->M_company_to->get_tables($tables,$search,$isWhere); // Mengambil data dari database
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');  // Memberi tahu browser bahwa data dalam bentuk format json 
        echo $this->M_company_to->get_tables_where($tables,$search,$where,$isWhere); // Mengambil data dari database     
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_company_to->Max_data($mdate,'tb_company_to')->row(); // Mengambil row dari database           
        ...
        $this->M_company_to->Input_Data($post_datamerge,'tb_company_to'); // Kirim hasil gabungan data ke model untuk insert tb_company_to

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))    
        {
            $this->upload_file_attach('file',$hdrid,'tb_company_to');  // Upload file attach ke table tb_company_to     
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_company_to->Update_Data($where,$post_datamerge,'tb_company_to'); // Kirim hasil gabungan,kondisi sesuai hdrid data ke model untuk update tb_company_to
    ...
    public function ajax_delete()
	{
        ...
        $this->M_company_to->Delete_Data($where,'tb_company_to'); // Kirim kondisi where,di table tb_company_to untuk query di model
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_company_to->ajax_getbyhdrid($hdrid,'tb_company_to')->row(); // Kirim $hdrid,di table tb_company_to untuk query di model
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $this->M_company_to->Update_Data($where,$data_update,$table); // Kirim semua parameter ke model
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_company_to'); // Kembali ke halaman C_company_to
			...
			if ( ! $this->upload->do_upload('excel')){
                    ...
                    if ($index > 0) {
                        ...
                        // ********************* 0. Generate nomor transaksi  *********************          
                            ...
                            if($index==0){
                                ...
                                $hdrid2=$this->M_company_to->Max_data($mdate,'tb_company_to')->row();  
                                ...
                                $mdate="TR".mdate('%Y%m',time());
                                $hdrid2=$this->M_company_to->Max_data($mdate,'tb_company_to')->row();
                                $hdrid22=$hdrid2->hdrid;
                                $hdrid3="TR".(intval(substr($hdrid22,2,10))+1); // maka naik 1 level
                                // var_dump($hdrid2);
                            ...
                            $resultData[$index]['company_to'] = ucwords($value['C']);
                            $resultData[$index]['company_address'] = ucwords($value['D']);
                            $resultData[$index]['company_address1'] = ucwords($value['E']);
                            $resultData[$index]['company_address2'] = ucwords($value['F']);
                            $resultData[$index]['attention'] = ucwords($value['G']);
                            $resultData[$index]['attention_departement'] = ucwords($value['H']);
                            $resultData[$index]['email_and_telp'] = ucwords($value['I']);
				...
				if (count($resultData) != 0) {
					$result = $this->M_company_to->insert_batch('tb_company_to',$resultData);
					if ($result > 0) {
