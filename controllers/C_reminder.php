...
class C_reminder extends CI_Controller {
	 ...
	 **/
     ...
     public function __construct(){
        ...
        $this->load->model('M_reminder'); // Load model
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_reminder->get_tb_reminder();  // Menarik data dan menampung nya menjadi groups
        // var_dump($data);
        // die;
    ...
    /// @attention Menampilkan data tanpa kondisi
     function view_data()
     {
        ...
        $tables = "tb_reminder"; // Table yang digunakan        
        $search = array('hdrid','item','durasi');  // Column column yang akan muncul di feature search datatable
         ...
         echo $this->M_reminder->get_tables($tables,$search,$isWhere); // Mengambil data dari database
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json'); // Memberi tahu browser bahwa data dalam bentuk format json
        echo $this->M_reminder->get_tables_where($tables,$search,$where,$isWhere); // Mengambil data dari database
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_reminder->Max_data($mdate,'tb_reminder')->row();   // Mengambil row dari database        
        ...
        $this->M_reminder->Input_Data($post_datamerge,'tb_reminder'); // Mengirim parameter ke model untuk query 

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))
        {
            $this->upload_file_attach('file',$hdrid,'tb_reminder'); // Upload attach file sesuai table                   
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_reminder->Update_Data($where,$post_datamerge,'tb_reminder'); // Mengirim parameter ke model
    ...
    public function ajax_delete()
	{
        ...
        $this->M_reminder->Delete_Data($where,'tb_reminder'); // Mengirim parameter ke model 
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_reminder->ajax_getbyhdrid($hdrid,'tb_reminder')->row(); // Mengirim parameter ke model dan menampung nya menjadi variable
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $this->M_reminder->Update_Data($where,$data_update,$table); // Kirim semua parameter ke model
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_reminder'); // Kembali ke halaman C_reminder
				...
				foreach ($sheetData as $key => $value) {
                     ...
                     if ($index > 1 && ucwords($value['D'])!='' ) {
                        ...
                        // ********************* 0. Generate nomor transaksi  *********************          
                            ...
                            $resultData[$index]['item'] = ucwords($value['A']);// Isi item dari column A excel 
                            $resultData[$index]['durasi'] = ucwords($value['B']);  // Isi durasi dari column B excel                           
