...
class C_bank_name extends CI_Controller {
    ...
    public function __construct(){
        ...
        $this->load->model('M_bank_name'); // Load model
        ...
        $Hak_akses = $this->UserModel->get_controller_access($this->session->userdata('role_id'),'C_bank_name'); 
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_bank_name->get_tb_bank_name(); // Menarik dan menampung ke data groups
        // var_dump($data);
        // die;
		...
		// // $this->load->view('bank_name/V_bank_name',$data); // Tampil data
        $this->load->view('bank_name/V_bank_name',$data); // Tampil data
    ...
    function view_data()
    {
        ...
        echo $this->M_bank_name->get_tables($tables,$search,$isWhere); // Mengambil data dari database
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');  // Memberi tahu browser bahwa data dalam bentuk format json 
        echo $this->M_bank_name->get_tables_where($tables,$search,$where,$isWhere); // Mengambil data dari database     
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_bank_name->Max_data($mdate,'tb_bank_name')->row(); // Mengambil row dari database           
        ...
        $this->M_bank_name->Input_Data($post_datamerge,'tb_bank_name'); // Kirim hasil gabungan data ke model untuk insert tb_bank_name

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))    
        {
            $this->upload_file_attach('file',$hdrid,'tb_bank_name');  // Upload file attach ke table tb_bank_name     
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_bank_name->Update_Data($where,$post_datamerge,'tb_bank_name'); // Kirim hasil gabungan,kondisi sesuai hdrid data ke model untuk update tb_bank_name
    ...
    public function ajax_delete()
	{
        ...
        $this->M_bank_name->Delete_Data($where,'tb_bank_name'); // Kirim kondisi where,di table tb_bank_name untuk query di model
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_bank_name->ajax_getbyhdrid($hdrid,'tb_bank_name')->row(); // Kirim $hdrid,di table tb_bank_name untuk query di model
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $this->M_bank_name->Update_Data($where,$data_update,$table); // Kirim semua parameter ke model
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_bank_name'); // Kembali ke halaman C_bank_name
			...
			if ( ! $this->upload->do_upload('excel')){
                    ...
                    if ($index > 1 && ucwords($value['A'])!='' ) {
                        ...
                        // ********************* 0. Generate nomor transaksi  *********************          
                            ...
                            if($index==2){
                                ...
                                $hdrid2=$this->M_bank_name->Max_data($mdate,'tb_bank_name')->row();  
				...
				if (count($resultData) != 0) {
					$result = $this->M_bank_name->insert_batch('tb_bank_name',$resultData);
					if ($result > 0) {
