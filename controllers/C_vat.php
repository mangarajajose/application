...
class C_vat extends CI_Controller {
    ...
    public function __construct(){
        ...
        $this->load->model('M_vat'); // Load model
        ...
        $Hak_akses = $this->UserModel->get_controller_access($this->session->userdata('role_id'),'C_vat'); 
	...
	public function Index()
	{
        ...
        $data['groups'] = $this->M_vat->get_tb_vat(); // Menarik dan menampung ke data groups
        // var_dump($data);
        // die;
		...
		// // $this->load->view('vat/V_vat',$data); // Tampil data
        $this->load->view('vat/V_vat',$data); // Tampil data
    ...
    function view_data()
    {
        ...
        echo $this->M_vat->get_tables($tables,$search,$isWhere); // Mengambil data dari database
    ...
    function view_data_where()
    {
        ...
        // $isWhere = 'artikel.deleted_at IS NULL';
        header('Content-Type: application/json');  // Memberi tahu browser bahwa data dalam bentuk format json 
        echo $this->M_vat->get_tables_where($tables,$search,$where,$isWhere); // Mengambil data dari database     
    ...
    public function ajax_add()
	{
        ...
        $hdrid2=$this->M_vat->Max_data($mdate,'tb_vat')->row(); // Mengambil row dari database           
        ...
        $this->M_vat->Input_Data($post_datamerge,'tb_vat'); // Kirim hasil gabungan data ke model untuk insert tb_vat

        // ********************* 6. Upload file jika ada  *********************   
        if(!empty($_FILES['file']['name']))    
        {
            $this->upload_file_attach('file',$hdrid,'tb_vat');  // Upload file attach ke table tb_vat     
        }
    ...
    public function ajax_update()
	{
        ...
        $this->M_vat->Update_Data($where,$post_datamerge,'tb_vat'); // Kirim hasil gabungan,kondisi sesuai hdrid data ke model untuk update tb_vat
    ...
    public function ajax_delete()
	{
        ...
        $this->M_vat->Delete_Data($where,'tb_vat'); // Kirim kondisi where,di table tb_vat untuk query di model
    ...
    function ajax_getbyhdrid(){      
        ...
        $data=$this->M_vat->ajax_getbyhdrid($hdrid,'tb_vat')->row(); // Kirim $hdrid,di table tb_vat untuk query di model
    ...
    function upload_file_attach($filename,$hdrid,$table){
        ...
        if(!empty($_FILES[$filename]['name']))
        {
            ...
            if ( ! $this->upload->do_upload($filename)){
                ...
                $this->M_vat->Update_Data($where,$data_update,$table); // Kirim semua parameter ke model
    ...
    public function import() {
		...
		if ($_FILES['excel']['name'] == '') {
            ...
            redirect('C_vat'); // Kembali ke halaman C_vat
			...
			if ( ! $this->upload->do_upload('excel')){
                    ...
                    if ($index > 1 && ucwords($value['A'])!='' ) {
                        ...
                        // ********************* 0. Generate nomor transaksi  *********************          
                            ...
                            if($index==2){
                                ...
                                $hdrid2=$this->M_vat->Max_data($mdate,'tb_vat')->row();  
				...
				if (count($resultData) != 0) {
					$result = $this->M_vat->insert_batch('tb_vat',$resultData);
					if ($result > 0) {
