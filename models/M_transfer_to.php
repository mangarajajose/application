...
class M_transfer_to extends CI_Model {
      
       /// @see get_tables()
   /// @note untuk mendapat data table pada transfer to
   /// @attention data di tarik ke transfer to
   function get_tables($tables,$cari,$iswhere)
        ...
        /// @note untuk mencari data table transfer to
        /// @attention cari data di transfer to
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM transfer_to_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from transfer_to_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('transfer_to_approver',$data);
   ...
   /// @attention data akan ditarik transfer_to
   public function get_tb_transfer_to()
   {
   ...
   /// @attention data bisa dicari search
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_transfer_to', ['hdrid' => $transfer_to_id])->row_array();
