...
class M_vat extends CI_Model {
      
       /// @see get_tables()
   /// @note untuk mendapat data table pada vat
   /// @attention data di tarik ke vat
   function get_tables($tables,$cari,$iswhere)
        ...
        /// @note untuk mencari data table vat
        /// @attention cari data di vat
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,description
     FROM vat_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from vat_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('vat_approver',$data);
   ...
   /// @attention data akan ditarik vat
   public function get_tb_vat()
   {
   ...
   /// @attention data bisa dicari search
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_vat', ['hdrid' => $vat_id])->row_array();
