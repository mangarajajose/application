...
class M_duedate extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM duedate_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from duedate_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('duedate_approver',$data);
   ...
   // Get Data Group duedate

   /// @see get_tb_duedate()
   /// @note
   /// @attention
   public function get_tb_duedate()
   {
    $query =  $this->db->get('tb_duedate')->result();
    return $query;
   }

   // Get Data report no By duedate ID
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_duedate', ['hdrid' => $duedate_id])->row_array();
