...
class M_sys_user extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM sys_user_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from sys_user_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('sys_user_approver',$data);
   ...
   // Get Data Group sys_user

    /// @see get_tb_sys_user()
   /// @note 
   /// @attention
   public function get_tb_sys_user()
   {
    $query =  $this->db->get('tb_sys_user')->result();
    return $query;
   }

   // Get Data report no By sys_user ID
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_sys_user', ['hdrid' => $sys_user_id])->row_array();
