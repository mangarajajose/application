...
class M_role_form extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM role_form_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from role_form_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('role_form_approver',$data);
   ...
   // Get Data Group role_form

   /// @see get_tb_role_form()
   /// @note 
   /// @attention 
   public function get_tb_role_form()
   {
    $query =  $this->db->get('tb_role_form')->result();
    return $query;
   }

   // Get Data report no By role_form ID
   ...
   /// @attention 
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_role_form', ['hdrid' => $role_form_id])->row_array();
