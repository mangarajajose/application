...
class M_dn2 extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,description
     FROM dn2_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from dn2_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('dn2_approver',$data);
   ...
   /// @note
   /// @attention
   public function get_tb_dn2()
   {
    $query =  $this->db->get('tb_dn2')->result();
    return $query;
   }

   // Get Data report no By dn2 ID
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_dn2', ['hdrid' => $dn2_id])->row_array();
