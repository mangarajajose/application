...
class M_cn2 extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,description
     FROM cn2_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from cn2_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('cn2_approver',$data);
   ...
   /// @attention data akan ditarik cn2
   public function get_tb_cn2()
   {
    $query =  $this->db->get('tb_cn2')->result();
    return $query;
   }
   ...
   /// @attention data bisa dicari search
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_cn2', ['hdrid' => $cn2_id])->row_array();
