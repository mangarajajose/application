...
class M_cn3 extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,description
     FROM cn3_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from cn3_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('cn3_approver',$data);
   ...
   /// @note
   /// @attention
   public function get_tb_cn3()
   {
    $query =  $this->db->get('tb_cn3')->result();
    return $query;
   }

   // Get Data report no By cn3 ID
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_cn3', ['hdrid' => $cn3_id])->row_array();
