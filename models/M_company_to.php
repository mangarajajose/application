...
class M_company_to extends CI_Model {
      
       /// @see get_tables()
   /// @note untuk mendapat data table pada company to
   /// @attention data di tarik ke company to
   function get_tables($tables,$cari,$iswhere)
        ...
        /// @note untuk mencari data table company to
        /// @attention cari data di company to
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM company_to_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from company_to_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('company_to_approver',$data);
   ...
   /// @attention data akan ditarik company_to
   public function get_tb_company_to()
   {
   ...
   /// @attention data bisa dicari search
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_company_to', ['hdrid' => $company_to_id])->row_array();
