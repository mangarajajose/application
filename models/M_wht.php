...
class M_wht extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,description
     FROM wht_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from wht_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('wht_approver',$data);
   ...
   /// @note 
   /// @attention
   public function get_tb_wht()
   {
    $query =  $this->db->get('tb_wht')->result();
    return $query;
   }

   // Get Data report no By wht ID
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_wht', ['hdrid' => $wht_id])->row_array();
