...
class M_tipe_transfer extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM tipe_transfer_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from tipe_transfer_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('tipe_transfer_approver',$data);
   ...
   // Get Data Group tipe_transfer

    /// @see get_tb_tipe_transfer()
   /// @note 
   /// @attention
   public function get_tb_tipe_transfer()
   {
    $query =  $this->db->get('tb_tipe_transfer')->result();
    return $query;
   }

   // Get Data report no By tipe_transfer ID
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_tipe_transfer', ['hdrid' => $tipe_transfer_id])->row_array();
