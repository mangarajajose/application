...
class M_cn extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,description
     FROM cn_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from cn_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('cn_approver',$data);
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_cn', ['hdrid' => $cn_id])->row_array();
