...
class M_cicilan_dn extends CI_Model {
  ...
  /// @see Input_Data()
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM cicilan_dn_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from cicilan_dn_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('cicilan_dn_approver',$data);
   ...
   // Get Data Group cicilan_dn

   /// @see get_tb_dn()
   /// @note untuk mencari tb_dn
   /// @attention
   public function get_tb_cicilan_dn()
   {
    $query =  $this->db->get('tb_cicilan_dn')->result();
    return $query;
   }

   // Get Data report no By cicilan_dn ID
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_cicilan_dn', ['hdrid' => $cicilan_dn_id])->row_array();
