...
class M_sys_role extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM sys_role_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from sys_role_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('sys_role_approver',$data);
   ...
   // Get Data Group sys_role

    /// @see get_tb_sys_role()
   /// @note 
   /// @attention
   public function get_tb_sys_role()
   {
    $query =  $this->db->get('tb_sys_role')->result();
    return $query;
   }

   // Get Data report no By sys_role ID
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_sys_role', ['hdrid' => $sys_role_id])->row_array();
