...
class M_nama_account extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM nama_account_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from nama_account_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('nama_account_approver',$data);
   ...
   // Get Data Group nama_account

   /// @see get_tb_nama_account()
   /// @note
   /// @attention
   public function get_tb_nama_account()
   {
    $query =  $this->db->get('tb_nama_account')->result();
    return $query;
   }

   // Get Data report no By nama_account ID
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_nama_account', ['hdrid' => $nama_account_id])->row_array();
