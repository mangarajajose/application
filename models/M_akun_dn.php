...
class M_akun_dn extends CI_Model {
      
       /// @see get_tables()
   /// @note untuk mendapat data table pada akun dn
   /// @attention data di tarik ke akun dn
   function get_tables($tables,$cari,$iswhere)
        ...
        /// @note untuk mencari data table akun dn
        /// @attention cari data di akun dn
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM akun_dn_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from akun_dn_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('akun_dn_approver',$data);
   ...
   /// @attention data akan ditarik akun_dn
   public function get_tb_akun_dn()
   {
    $query =  $this->db->get('tb_akun_dn')->result();
    return $query;
   }
   ...
   /// @attention data bisa dicari search
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_akun_dn', ['hdrid' => $akun_dn_id])->row_array();
