...
class M_reminder extends CI_Model {
   ...
   public function Send_mail($hdrid)
   {
    ...
    $query =$this->db->query("
         ...
         ,date_approve
         ,description
     FROM reminder_approver
     where hdrid='WTR2010009' and level_approve=(select min(level_approve) from reminder_approver where hdrid='WTR2010009' and date_approve is null)");
   ...
   if ($query->num_rows()>0) {
      ...
      // Update progress waiting approve
       ...
       $this->db->where($where);
       $this->db->update('reminder_approver',$data);
   ...
   // Get Data Group reminder

   /// @see get_tb_reminder()
   /// @note
   /// @attention
   public function get_tb_reminder()
   {
    $query =  $this->db->get('tb_reminder')->result();
    return $query;
   }

   // Get Data report no By reminder ID
   ...
   /// @attention
    public function get_hdrid($table,$column,$value)
    {
        ...
        // return $this->db->get_where('tb_reminder', ['hdrid' => $reminder_id])->row_array();
